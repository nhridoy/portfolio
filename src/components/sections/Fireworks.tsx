...
function FireworkItem({
  item,
  targetX,
  targetY,
  scrollYProgress,
}: FireworkItemProps) {
  const x = useTransform(scrollYProgress, [-0.3, 1], [0, targetX], {
    clamp: true,
  });
  const y = useTransform(scrollYProgress, [-0.3, 1], [0, targetY], {
    clamp: true,
  });
...
export function Fireworks() {
  ...
  const itemPositions = useMemo<ItemPosition[]>(() => {
    return allItems.map((item, index) => {
      const angle =
        (index / allItems.length) * 360 + (Math.random() - 0.5) * 60;
