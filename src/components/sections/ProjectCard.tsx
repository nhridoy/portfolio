...
const ProjectCard = ({ item }: { item: (typeof PROJECTS)[0] }) => {
  return (
    <AnimatedLink
      href={item.link}
      external
      className="absolute size-full group"
    >
      <Image
        src={item.image}
        alt={item.title}
        fill
        className="absolute object-cover object-top transition-transform duration-500 group-hover:scale-110"
      />
